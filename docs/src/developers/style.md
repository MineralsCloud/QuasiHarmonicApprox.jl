...
!!! info
    The style guide is always a work in progress, and not all QuasiHarmonicApprox code
    follows the rules. When modifying QuasiHarmonicApprox, please fix the style violations
...
`QuasiHarmonicApprox` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/QuasiHarmonicApprox.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `QuasiHarmonicApprox` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `QuasiHarmonicApprox` have
